...
describe('EventEditor', () => {
  ...
  it('renders nothing when weekId is null', () => {
    const { container } = render(
      <EventEditor event={null} weekId={null} open onSave={vi.fn()} onDelete={vi.fn()} onClose={vi.fn()} />,
    )
  ...
  it('pre-fills fields when editing existing event', () => {
    render(
      <EventEditor event={existingEvent} weekId="week-1" open onSave={vi.fn()} onDelete={vi.fn()} onClose={vi.fn()} />,
    )
  ...
  it('starts empty for new event', () => {
    render(
      <EventEditor event={null} weekId="week-1" open onSave={vi.fn()} onDelete={vi.fn()} onClose={vi.fn()} />,
    )
  ...
  it('calls onSave with form data when Add clicked (new event)', () => {
    const onSave = vi.fn()
    render(
      <EventEditor event={null} weekId="week-1" open onSave={onSave} onDelete={vi.fn()} onClose={vi.fn()} />,
    )
    fireEvent.change(screen.getByPlaceholderText('e.g. Brushy Creek'), { target: { value: 'New Group' } })
    fireEvent.change(screen.getByPlaceholderText('10'), { target: { value: '15' } })
    fireEvent.click(screen.getByText('Add'))
    expect(onSave).toHaveBeenCalledWith('week-1', '', expect.objectContaining({ groupName: 'New Group', headcount: 15 }))
  })

  it('uses headcount 10 when blank', () => {
    const onSave = vi.fn()
    render(
      <EventEditor event={null} weekId="week-1" open onSave={onSave} onDelete={vi.fn()} onClose={vi.fn()} />,
    )
    fireEvent.change(screen.getByPlaceholderText('e.g. Brushy Creek'), { target: { value: 'Group' } })
    fireEvent.click(screen.getByText('Add'))
    expect(onSave).toHaveBeenCalledWith('week-1', '', expect.objectContaining({ headcount: 10 }))
  })
  ...
  it('calls onDelete and onClose when Delete clicked and confirmed', () => {
    ...
    const onClose = vi.fn()
    render(
      <EventEditor event={existingEvent} weekId="week-1" open onSave={vi.fn()} onDelete={onDelete} onClose={onClose} />,
    )
