      ...
      <div className="flex flex-1 flex-col gap-2 overflow-hidden">
        {weeks.map((w) => (
          <WeekColumn
            ...
            dbEventsDisabled={dbEventsDisabled}
          />
        ))}
      </div>
    );
...
function WeekColumn({
  ...
  const weekId =
    weekData?.id ??
    (weekNumber !== undefined ? `week-${weekNumber}` : "week-unknown");
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden rounded border border-(--border) bg-(--surface) text-xs">
      <div
