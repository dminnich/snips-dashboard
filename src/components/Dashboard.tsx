...
export function Dashboard() {
  ...
  const { months, weeks, updateMonth, updateWeek, addEvent, updateEvent, deleteEvent, toolbar } = useLocalData()
  ...
  const leftMonths = MONTHS_LEFT.map(
    (name) => months.find((m) => m.name === name) ?? {
      id: name.toLowerCase(), name, content: '', subtitle: '', specialEvents: '',
    },
  )
