...
export function EventEditor({ event, weekId, open, onSave, onDelete, onClose }: EventEditorProps) {
  const isNew = !event
  ...
  const handleSave = () => {
    const hc = headcount === '' ? 10 : Number(headcount)
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        <div>
          <label className="mb-1 block text-xs font-semibold text-(--text-secondary)">Group Name</label>
          <input
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            <label className="mb-1 block text-xs font-semibold text-(--text-secondary)">Headcount</label>
            <input
              type="text"
              inputMode="numeric"
          ...
          <div className="flex-1">
            <label className="mb-1 block text-xs font-semibold text-(--text-secondary)">Housing</label>
            <input
              className="w-full rounded border border-(--input-border) bg-(--input-bg) px-3 py-2 text-sm text-(--text) placeholder:text-(--placeholder)"
              value={housing}
