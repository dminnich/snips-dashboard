import { useRef, useCallback, useLayoutEffect, type FormEvent, type ClipboardEvent } from 'react'
    ...
    <div className="overflow-hidden rounded-lg border border-(--input-border) bg-(--input-bg)">
      <div className="flex flex-wrap items-center gap-0.5 border-b border-(--input-border) bg-(--surface) px-1.5 py-1">
        ...
        <select
          ...
          onChange={(e: FormEvent<HTMLSelectElement>) => exec('fontSize', (e.target as HTMLSelectElement).value)}
          defaultValue=""
        >
