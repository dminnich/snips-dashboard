...
describe('MonthEditor', () => {
  ...
  it('pre-fills fields with month data when open', () => {
    render(<MonthEditor month={baseMonth} open onSave={vi.fn()} onClose={vi.fn()} />)
  ...
  it('calls onSave with updated data and onClose when Save clicked', () => {
    const onSave = vi.fn()
    const onClose = vi.fn()
    render(<MonthEditor month={baseMonth} open onSave={onSave} onClose={onClose} />)
    fireEvent.change(screen.getByDisplayValue('old subtitle'), { target: { value: 'new subtitle' } })
    fireEvent.click(screen.getByText('Save'))
    expect(onSave).toHaveBeenCalledWith('january', expect.objectContaining({ subtitle: 'new subtitle' }))
    expect(onClose).toHaveBeenCalledOnce()
  })

  it('calls onClose when Cancel clicked', () => {
    const onClose = vi.fn()
    render(<MonthEditor month={baseMonth} open onSave={vi.fn()} onClose={onClose} />)
